...
class _DashboardPageState extends State<DashboardPage> {
  ...
  /// ================= TIMER =================
  Timer? _timer;
  int sisaDetik = 0; // 🔄 DIUBAH: Menggunakan hitungan detik untuk timer yang presisi
  ...
  /// 🛠️ MEMBERSIHKAN TIMER SAAT WIDGET DIHANCURKAN
  @override
  void dispose() {
    _timer?.cancel();
    super.dispose();
  ...
  Future<void> _tryStartTimer() async {
    ...
    setState(() {
      // 🔄 DIUBAH: Konversi durasi menit dari DB menjadi detik (Menit * 60)
      sisaDetik = durasiMenit * 60;
    });

    // 🔄 DIUBAH: Timer berjalan secara periodik setiap 1 detik untuk hitung mundur yang aktif
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) async {
      if (sisaDetik <= 0) {
        ...
        setState(() {
          sisaDetik--; // 🔄 DIUBAH: Mengurangi sisa waktu per detik
        });
      }
    });
  }
  ...
  /// ================= FORMAT TIMER =================
  // 🔄 DIUBAH: Memformat total detik menjadi string digital clock "MM:SS"
  String formatTimer(int totalDetik) {
  ...
  Future<void> emergencyCutOff() async {
    ...
    setState(() {
      sisaDetik = 0; // 🔄 DIUBAH: Reset ke 0 detik
    });
            ...
            SizedBox(
              ...
              child: isLoading
                ...
                style: TextStyle(
                  ...
                  : (sisaDetik != 0 // 🔄 DIUBAH: Menggunakan sisaDetik
                  ? Text(
                'Sisa Waktu: ${formatTimer(sisaDetik)}',
                style: const TextStyle(
                  fontSize: 20, // Diperbesar sedikit agar tampilan timer lebih kontras
                  fontWeight: FontWeight.bold,
                  color: terracottaDark,
                  letterSpacing: 1.2,
                ),
